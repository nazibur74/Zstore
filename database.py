...
def create_db():
    ...
    # Existing Tables
    cur.execute(
        "CREATE TABLE IF NOT EXISTS Employee(ID INTEGER PRIMARY KEY AUTOINCREMENT, Gender text, Contact text, Name text, DateofBirth text, DateofJoining text, Email text, Password text, UserType text, Address text, Salary text)")
    con.commit()
